...
Operator-rigged four-horse race. The operator picks the winner; the spectator screen plays a 20-second carnival horse race that always ends with that horse crossing first.
...
4. Click **Start race**. Jockeys gallop for 20 seconds, then the chosen horse hits the finish line.
5. **Reset** returns the horses to the starting line.

Starting again (same winner or a new one) uses a new seed, so lead changes look different.

## Spectator

- **Idle:** four horses on the starting line, no winner callout.
...
The spectator does not stream horse positions. Every screen derives progress from the shared start time, duration, winner, and seed.
